            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="text-sm text-gray-500 mt-3"> <i
                            class="mr-2 icono fa-solid fa-screwdriver-wrench text-green-600"></i><strong
                            class="capitalize">{{ __('maintenance protocols') }}</strong>:
                        {{ $prototype->protocols->count() }} </p>
                    <p class="text-sm text-gray-500 mt-3"> <i
                            class="mr-2 icono text-blue-500 fa-solid fa-file-contract"></i><strong
                            class="capitalize">{{ __('feature') }}</strong>:
                    ...
                    <p class="text-sm text-gray-500 mt-3"> <i
                            class="mr-2 icono text-blue-500 fa-solid fa-file-contract"></i><strong
                            class="capitalize">{{ __('zones') }}</strong>:
                        {{ $prototype->zones() }}
                    </p>
                </div>
            </div>
                ...
                <div class="card-body">
                    @foreach ($prototype->protocols as $protocol)
                        <div class="grid grid-cols-3 gap-3">
                            <div>
                                <p class="text-gray-600 font-semibold text-sm"><strong
                                        class="capitalize">{{ __('specialty') }}</strong>:
                                    {{ $protocol->specialty()->name }}</p>
                                <p class="text-gray-600 font-semibold text-sm"><strong
                                        class="capitalize">{{ __('task type') }}</strong> :
                                    {{ $protocol->typeTask()->name }}</p>
                            </div>
                            <div>
                                <p class="text-gray-600 font-bold text-sm">{{ __('position') }} :
                                    {{ $protocol->position }}</p>
                                <p class="text-gray-600 font-bold text-sm">{{ __('task') }} : {{ $protocol->task }}
                                </p>
                                <p class="text-gray-600 font-semibold text-xs">{{ __('detail') }} :
                                    {{ $protocol->detail }}</p>
                            </div>
                            <div>
                                <p class="text-gray-600 font-bold text-sm">{{ __('detail') }} :
                                    {{ $protocol->permissions }}</p>
                                <p class="text-gray-600 font-semibold text-xs">{{ __('security') }} :
                                    {{ $protocol->security }}</p>
                                </p>
                                <p class="text-gray-600 font-semibold text-xs">{{ __('condition') }} :
                                    {{ $protocol->conditions }}</p>
                                <p class="text-gray-600 font-bold text-sm">{{ __('frecuency') }} :
                                    {{ $protocol->frecuency }} veces al
                                    año</p>
                                <p class="text-gray-600 font-bold text-sm">{{ __('duration') }} :
                                    {{ $protocol->duration }} horas</p>
                                <p class="text-gray-600 font-bold text-sm">{{ __('workers') }} :
                                    {{ $protocol->workers }}
                                </p>
                            </div>
                        </div>
                        ...
                        @foreach ($prototype->features as $feature)
                            <div>
                                <p class="text-gray-600 font-semibold text-sm">
                                    ...
                                    : <span class="text-xs"> {{ $feature->unit }}</span>
                                </p>
