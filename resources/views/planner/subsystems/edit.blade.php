<x-app-layout>
    <div class="container my-4">
        <form action="{{ route('subsystems.update',$subsystem->id) }}" method="POST" class="max-w-md mx-auto rounded-lg shadow-lg">
            @csrf
            @method('PUT')
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        <div class="">
                            <x-jet-label class="italic my-2 capitalize" value="{{ __('sistema') }}" for="system" />
                            <select name="system_id" class="w-full rounded-lg">
                                @foreach ($systems as $system)
                                    <option value="{{ $system->id }}" @if($system->id ==$subsystem->system_id) selected @endif>{{ $system->name }}</option>
                                @endforeach
                        ...
                        <div class="mb-4">
                            <x-jet-label class="italic my-2 capitalize" value="{{ __('subsistema') }}" for="name" />
