            ...
            <div class="card max-w-xl">
                <div class="card-body">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-xs">
                        <div>
                            <div class="mb-4">
                                <x-jet-label class="italic my-2 capitalize" value="{{ __('Nombre de Equipo') }}"
                                    for="name" />
                                <x-jet-input type="text" name="name" class="w-full "
                                    placeholder="{{ __('nombre del equipo') }}" value="{{ old('name', $team->name) }}" />
                                <x-jet-input-error for="name" />
                            </div>

                            <div class="mb-4">
                                <x-jet-label class="italic my-2 capitalize" value="{{ __('Responsable') }}"
                                    for="user_id" />
                                <select name="user_id" class="w-full rounded-lg">
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            @if ($user->id == $team->user_id) selected @endif>{{ $user->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <div class="mb-4">
                                <x-jet-label class="italic my-2 capitalize" value="{{ __('Especialidad') }}"
                                    for="name" />
                                <select name="specialty_id" class="w-full rounded-lg">
                                    @foreach ($specialties as $specialty)
                                        <option value="{{ $specialty->id }}"
                                            @if ($specialty->id == $team->specialty_id) selected @endif>{{ $specialty->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                <x-jet-label class="italic my-2 capitalize" value="{{ __('Zona') }}"
                                    for="name" />
                                <select name="zone_id[]" class="select w-full rounded-lg" multiple="multiple">
                                    @foreach ($zones as $zone)
                                        <option value="{{ $zone->id }}"
                                            @if ($zone->id == $team->zone_id) selected @endif>{{ $zone->name }}
                                        </option>
                                    @endforeach
                                </select>
