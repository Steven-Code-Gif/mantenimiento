<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow-xl sm:rounded-lg p-6 my-8 max-w-6xl mx-auto">
            <h1 class="text-2xl text-center text-gray-500 uppercase font-bold">{{ __('lista de suministros') }}</h1>
            <div class="flex items-center justify-end mb-3">
                <a href="{{ route('supplies.create') }}" class="px-3 py-2 bg-green-500 text-white rounded-lg hover:bg-green-400">
                    <i class="fas fa-bottle-droplet"></i>
                    {{ __('agregar suministros') }}
                </a>
            </div>
            <table id="supply" class="">
                <thead>
                    <tr class="text-gray-600">
                        ...
                        <th class="capitalize">{{ __("unidad") }}</th>
                        <th class="capitalize">{{ __("marca") }}</th>
                        <th class="capitalize">{{ __("suministro") }}</th>
                    ...
                    @foreach ($supplies as $supply)
                        <tr class="text-gray-500 odd:bg-slate-100">
                            ...
                            <td class="flex items-center justify-between gap-3">
                                <a href="{{ route('supplies.edit',$supply->id) }}" title="{{ __('editar suministro').$supply->name }}"><i class="icono text-green-500 fa-solid fa-pen-to-square"></i></a>
